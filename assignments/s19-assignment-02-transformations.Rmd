...
7. Fit the model using sporting league, and all adopted effects of transformed average player salary (based on result in Question 5) to predict variation in log-transformed FCI (Model 3). Again, use NBA as the reference group in this model. Report and interpret the back-transformed NHL coefficient from the fitted model.


## Models 4, 5, and 6: League and Player Salary (Interaction Effects)

Fit a series of interaction models using the effect of sporting league and the effects of average player salary to predict variation in log-transformed FCI.

- The first interaction model (Model 4) will include interaction effects between sporting league and only the linear effect of average player salary (but will still include the quadratic and cubic main-effect of transformed average player salary).
- The second interaction model (Model 5) will include interaction effects between sporting league and only the linear and quadratic effects of average player salary (but will still include the cubic main-effect of transformed average player salary). 
- The third interaction model (Model 6) will include interaction effects between sporting league and the linear, quadratic, and cubic effects of average player salary.

Remember that to include interaction effects when you have a categorical variable represented by more than one dummy variable, you need to create and include ALL product terms. (Hint: Model 6 will include 16 predictors.)

8. Use the test for nested models (the $\Delta F$ test) to test the necessity of including the different interaction components by comparing four models: the main-effects model fitted in Question 7 (Model 3); the interaction model with linear interaction effects (Model 4); the interaction model with linear and quadratic interaction effects (Model 5); and the interaction model with linear, quadratic, and cubic interaction effects (Model 6). Provide the output of the test, and explain which of the four models should be adopted based on the results of this test.
...
10. Create a table to display the regression results from a series of five fitted models: Models 1, 2, 3, 4, and the adopted interaction model (either Model 4, 5, or 6). This table should be similar in structure to Table 2 from Garcia, D. R., McIlroy, L., &amp; Barber, R. T. (2008). Starting Behind: A Comparative Analysis of the Academic Standing of Students Entering Charter Schools. _Social Science Quarterly, 89_(1), 199&ndash;216. Re-create this table as closely as you can (including the caption) using the output from the models you fitted above. **(2pts.)**

11. Write the fitted equation for the "final" adopted model using Equation Editor (or some other program that correctly typesets mathematical expressions).

12. Using the `ggplot()` function, create a plot of the fitted equation  for the "final" adopted model that allows you to interpret the effects of sporting league and average player salary on cost of attending a game. (Note: Make sure that you back-transform any log-transformed variables when you create this plot.)
