...
2. Regress the log-transformed FCI variable (using the natural logarithm) on payroll (Model 1). Report and interpret the slope coefficient from the fitted model.

3. Back-transform all the estimated coefficients from Model 1. Report and interpret the back-transformed slope coefficient. 

<br />

## Effect of Payroll and Playoffs on Log-Transformed FCI

4. Fit a model that includes the effects of payroll and whether or not the team made the playoffs in the previous year (Model 2). Also fit an intercept-only model (Model 0). Use the likelihood ratio test to examine which of these models to adopt. Provide the output of these tests, and comment on which model you adopt. **(2pts.)**
...
7. Interpret the effect associated with playoffs from this adopted model.

8. Back-transform the effect associated with playoffs from this adopted model. Report and interpret this back-transformed coefficient.
