...
This assignment is intended to have you build your understanding of using information criteria for model selection. Turn in a printed document that includes your responses to each of the questions on this assignment. Please adhere to the following guidelines for further formatting your assignment: 
...
Fit the same nine candidate models that Snipes and Taylor fitted in their analysis, but use the *wine.csv* data. In these models use wine rating (`rating`) as the outcome. In these models maintain `vintage` as a quantitative predictor. The point is not to replicate their exact data, but to use the same set of predictors---even though in our dataset the predictors have different levels (e.g., our data includes more regions than Snipes and Taylor's data), or we treat them differently (quantitative vs. categorical). By using a different set of data we can more rigorously evaluate the underlying working hypotheses. (Hint: For models that include the `region` predictor, it doesn't matter which region is the reference group; maybe let R pick that group by simply including `region` as a predictor.)
...
8. Based on previous literature, Snipes and Taylor hypothesized that price was an important predictor of wine quality. Based on your analyses, is price an important predictor of wine quality? Justify your response by referring to whether the empirical evidence has more support for models with the predictor of price or models without the price predictor.

9. Does the empirical evidence from your analysis support adopting more than one working hypothesis? Justify your response by referring to values reported in the table of model evidence.
