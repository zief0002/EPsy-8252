...
```{r lik_prof_01, echo=FALSE, fig.cap="Plot of the likelihood values for five candidate parameter values."}
# Create data
dat = data.frame(
  mu = c(10, 15, 20, 25, 30)
  ) %>%
...
```{r plot_example_1, fig.cap="Likelihood profile for several parameter values assuming a normal distribution and a standard deviation of 4.272."}
...
```{r echo=FALSE, fig.cap="Grid showing the combinations of parameter values used in the search space."}
...
```{r threed, echo=FALSE, fig.cap="Likelihood profile for the search space of both $\\mu$ and $\\sigma$ assuming a normal distribution."}
# Load library
library(plot3D)

scatter3D(x = dat$mu, y = dat$sigma, z = dat$L, pch = 18, cex = 2, 
    theta = 45, phi = 20, ticktype = "detailed",
    xlab = expression(mu), ylab = expression(sigma), zlab = "Likelihood",
    colkey = FALSE,
    colvar = dat$L, 
    col = ramp.col(col = c("#f6eff7", "#bdc9e1", "#67a9cf", "#1c9099", "#016c59"), 
                   n = 100, alpha = 1)
)
```
...
```{r plot_example_1_ln, fig.cap="Log-likelihood profile for several parameter values assuming a normal distribution and a standard deviation of 4.272."}
...
data.frame(
  ...
  kable(
    caption = "Toy data set that includes predictor (x) and outcome (y) values.",
    format = "html",
    escape = FALSE,
    align = c("c", "c"),
...
In practice, there are several issues with the grid search methods we have employed so far. The biggest is that you would not have any idea which values of $\beta_0$ and $\beta_1$ to limit the search space to. Essentially you would need to search an infinite number of values unless you could limit the search space in some way. For many common methods (e.g., linear regression) finding the ML estimates is mathematically pretty easy (if we know calculus; see the optional section [Using Calculus to Determine the MLEs](#way-too-much-math)). For more complex methods (e.g., mixed-effect models) there is not a mathematical solution. Instead, mathematics is used to help limit the search space and then a grid search is used to hone in on the estimates.
...
The parameter estimates using the criterion of maximum likelihood are presented in the first column of the table below. Comparing the coefficient estimates ($\hat\beta_0$ and $\hat\beta_1$) to those obtained through ordinary least squares (OLS; based on output from the `lm()` function---not shown), we find they are quite similar. The estimate of the residual standard error ($\sigma_{\epsilon}$), however, differs between the two estimation methods (although they are somewhat close in value).
...
data.frame(
  ...
  kable(
    caption = "Parameter estimates using maximum likelihood (ML) and ordinary least squares (OLS) estimation.",
    format = "html",
    escape = FALSE,
    align = c("c", "c", "c"),
...
$$
\hat\sigma_{\epsilon}= \sqrt{\frac{\left(Y_i - \hat{Y}_i\right)^2}{n-2}}
$$

And the residual standard error resulting from ML estimation is:
...
## Optional: Using Calculus to Determine the MLEs {#way-too-much-math}
