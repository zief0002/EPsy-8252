...
To allow for differences in individual students' growth patterns, we fit an unconditional growth model that includes random-effects for both the intercept and time predictor (`grade`). To do this we change the speicification of the random-effects in the `lmer()` function.
...
The empirical evidence supports only the inclusion of a random-effect of intercept, and in practice, we would likely drop the random-effect of slope from the model. This is consistent with the earlier evidence that we looked at when we summarized the random-effectsss of the slopes. There did not appear to be much variation in individuals' slopes, suggesting that they are not really different than the mean slope (fixed-effect); 

\vspace{3em}

__To facilitate understanding of the model that includes random-effects of both intercepts and slopes, I will interpret the output from Model 2 and continue to use it throughout this set of notes.__
...
&\qquad\qquad\qquad\qquad\qquad~= -0.493 + 1.62\big[\ln(\mathrm{Grade\mbox{-}Level}_{ij}-7)\big] \\
&\mathbf{Student~10:}\\
...
```{r fig.cap="Mean Profile and Two Individual Profiles Showing the Predicted Change in Vocabulary Score Over Time"}
...
We can use the random-effects estimates and the fixed-effects estimates to compute the estimated fitted equations for each of the students. We will also have to substitute the appropriate value for `female` (0 or 1) for the rspective students. For example, for Student 1, who is female:
...
While for Student 2, who is male:
...
$$
\begin{split}
\mathrm{Vocabulary~Score}_{ij} &= \bigg[\beta_0 + \beta_{2}(\mathrm{Female}_{\bullet j}) + b_{0j}\bigg] + \bigg[\beta_1 + b_{1j}\bigg]\big[\ln(\mathrm{Grade\mbox{-}Level}_{ij}-7) + \epsilon_{ij}\\
&= \beta_0 + \beta_{2}(\mathrm{Female}_{\bullet j}) + b_{0j} + \beta_1\big[\ln(\mathrm{Grade\mbox{-}Level}_{ij}-7)\big] + \\
&\qquad b_{1j}\big[\ln(\mathrm{Grade\mbox{-}Level}_{ij}-7)\big] + \epsilon_{ij} \\
...
```{r fig.width=8, fig.height=8, out.width='70%', fig.cap='Plots of the Level-1 Residuals for the Main Effects (Row 1) and Interaction Models (Row 2)', fig.pos='H'}
# Layout plots
(p1 | p2) / (p3 | p4)
```
...
We will also evaluate the normailty assumption for the Level-2 residuals.
...
```{r fig.width=8, fig.height=8, out.width='70%', fig.cap='Plots of the Level-2 Residuals for the Main Effects (Row 1) and Interaction Models (Row 2)', fig.pos='H'}
# Layout plots
(p5 | p6) / (p7 | p8)
```
