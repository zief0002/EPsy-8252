...
```{r out.width='3in', fig.cap="Six-year graduation rate as a function of median SAT score. The loess smoother  (blue, dashed line) and fitted regression line (red, dashed line) are also displayed.", echo=FALSE}
# Scatterplot
...
```{r out.width='50%', fig.cap='Scatterplot of the relationship between log-transformed median SAT score (base-2) and six-year graduation rate. The loess smoother (blue, solid line) and fitted regression line (red, dashed line) are also displayed.'}
...
```{r echo=FALSE, fig.width=8, fig.height=4, out.width='80%', fig.cap='Standardized residuals versus the fitted values for the models fitted with the log-2 predictor (left) and the log-10 predictor (right). In both plots the loess smoother (solid, blue line) is displayed along with the 95\\% confidence envelope (grey shaded area). The reference line of Y=0 is also displayed.'}
...
```{r out.width='50%', fig.cap='Plot of the predicted graduation rates as a function of median SAT score (in hundreds). This relationship is shown for both public (blue, solid line) and private (red, dashed line) school. '}
# Plot
ggplot(data = mn, aes(x = sat, y = grad)) +
	geom_point(alpha = 0) +
