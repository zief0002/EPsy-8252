...
In the previous set of notes, we determined that the relationship between SAT scores and graduation rates was curvilinear. We initially saw evidence of this curvilinearity in the scatterplot of graduation rates versus median SAT scores. After fitting a model assuming a linear relationship (syntax not shown), the plot of the standardized residuals versus the fitted values also suggests that the linearity assumption is not tenable.

```{r fig.cap="LEFT: Six-year graduation rate as a function of median SAT score. The loess smoother  (blue, dashed line) is also displayed. RIGHT: Scatterplot of the standardized residuals versus the fitted values for a model using median SAT scores to predict variation in six-year graduation rates. The loess smoother (solid, blue line) is displayed along with the 95% confidence envelope (grey shaded area). The reference line of Y=0 is also displayed.", echo=FALSE, fig.width=8, fig.height=4, out.width="80%"}
# Scatterplot
...
```{r echo=FALSE, fig.width=8, fig.height=4, out.width='80%', fig.cap='LEFT: Negative quadratic function of x. RIGHT: Logarithmic function of x.'}
fig_01 = data.frame(
...
```{r echo=FALSE, fig.width=8, fig.height=4, out.width='80%', fig.cap="Density plot of the median SAT scores (LEFT) and the base-2 log-transformed median SAT scores (RIGHT)."}
...
```{r out.width='40%', fig.cap='Scatterplot of six-year graduation rate versus base-2 log-transformed median SAT scores. The loess smoother (blue, solid line) is also displayed.'}
...
```{r out.width='40%', fig.cap="Plot of the standardized residuals versus the fitted values for a model fitted using the base-2 log-transformed median SAT scores. The loess smoother (solid, blue line) is displayed along with the 95% confidence envelope (grey shaded area). The reference line of Y=0 is also displayed.", message=FALSE, warning=FALSE}
...
```{r fig.cap='Plot of the predicted graduation rate as a function of median SAT score. The non-linearity in the plot indicates that there is a diminishing positive effect of SAT on graduation rates.'}
# Plot
...
# Model-level output
print(glance(lm.log10), width = Inf)
```
...
```{r echo=FALSE, fig.width=8, fig.height=4, out.width='80%', fig.cap="Scatterplot of the standardized residuals versus the fitted values for the model fitted using the base-2 log-transformed median SAT values (LEFT) and the base-10 log-transformed median SAT values (RIGHT). In both plots the loess smoother (solid, blue line) is displayed along with the 95% confidence envelope (grey shaded area). The reference line of Y=0 is also displayed."}
...
As with any base, using base-*e* results in the same model-level evidence ($R^2=.811$, *F*- and *p*-values).
...
We can get an approximatation for the size of the effect by using the mathematical shortcut of:
...
```{r fig.cap='Plot of predicted graduation rate as a function of median SAT score for public (blue, solid line) and private (red, dashed line) institutions.'}
# Plot
ggplot(data = mn, aes(x = sat, y = grad)) +
	geom_point(alpha = 0) +
	geom_function(
...
# Using the Likelihood Ratio Test to Adopt a Model

If you are interested in a more formal test to inform your model selection, we could use the likelihood ratio test to evaluate the evidence among a set of nested models. For example:
...
To choose from among thes models, we could evaluate the likelihood ratio tests for the following comparisons:

- Model 1 vs Model 2 (Is a model with SAT more likely to have produced the observed data than one without SAT?)
- Model 2 vs Model 4 (Is a model with both SAT and sector more likely to have produced the observed data than one with only SAT?)
- Model 3 vs Model 4 (Is a model with both SAT and sector more likely to have produced the observed data than one with only sector?)


You could also fit an interaction model and compare that to Model 4 to evaluate whether the SAT effect on graduation rates differs by sector (although we do not do that here).
...
Based on the results of the likelihood ratio tests, the empirical evidence overwhelmingly supports Model 2 (effect of median SAT on graduation rates) over Model 1; $LR=8.95\times10^{11}$, $\chi^2(1)=55.04$, $p<.001$. Note that this test can help us evaluate whether there is an effect, but CANNOT tell us whether that effect is nonlinear, nor whether a log function is appropriate to model the effect; for that you are reliant on the residuals! 

```{r}
lrtest(lm.2, lm.4)
```

Based on this analysis, the empirical evidence supports Model 4 over Model 2; $LR=268$, $\chi^2(1)=11.18$, $p=.001$. That is, we would adopt the model with effects of median SAT and sector over the model that only included the effect of median SAT.

```{r}
lrtest(lm.3, lm.4)
```

The last test allows us to evaluate the model that includes the effects of median SAT and sector to the model that only includes the effect of sector. In this comparison, the empirical evidence overwhelmingly supports Model 4; $LR=1.41\times10^{13}$, $\chi^2(1)=11.18$, $p=.001$.


Based on this analyses, we would adopt Model 4 (main effects of median SAT and sector). This model has the most empirical support given the data and candidate models we fitted.
...
```{r echo=FALSE, fig.cap='Comparison of the quadratic polynomial (blue, dashed line) and logarithmic (red, solid line) functions of X.'}
