...
```{r fig.width=8, fig.height=4, out.width='6in', fig.cap='Plots to Evaluate the Level-1 Residuals', fig.pos='H'}
...
```{r out.width='3in', fig.cap='Density Plot of the Estimated Random-Effects for Intercept', fig.pos='H'}
...
```{r fig.width=12, fig.height=4, out.width='6in', fig.cap='Plots to Evaluate the Level-1 and Level-2 Residuals', fig.pos='H'}
# Fit model
...
- Public schools have a log-transformed post-test language scores that is 0.04-points lower than private shcools, on average, controlling for differences in verbal IQ score and SES. Back-transfomrning this, we find that public schools have post-test language scores that are 4\% lower than private shcools, on average, controlling for differences in verbal IQ score and SES.


Note that the variance components change a lot after applying the log-transformationbecause the metric has changed: Raw scores versus log-transformed scores.
...
Remember, to obtain the baseline variance components we need to fit the model that only includes the fixed- and random-effects of intercet to predict variation in the outcome. We also need to square the values that `tidy()` produces because we need to work with variance components, not standard deviations.
...
Although these values are slightly diffeent from those obtained from the model with the raw outcome, they are similar in magnitude, namely:
