...
After creating the quadratic term in the data, we fit the regression model that includes both the linear and quadratic effects as predictors in the model. We then examine the coefficient-level output for the model.
...
data.frame(Model = c("Linear", "Quadratic")) %>%
  cbind(rbind(glance(lm.1), glance(lm.2))) %>%
  select(Model, R2 = r.squared, df1 = df, df2 = df.residual, F = statistic, p = p.value) %>%
  kable(
  caption = "Model-level output for the linear and quadratic models.",
...
glance(lm.3)
```
Based on the model-level output, we find the model now explains 89.7\% of the variation in graduation rates, which is more than we expect because of sampling variation, $F(3,29)=84.14$, $p<.001$.
...
```{r echo=FALSE}
data.frame(Model = c("Main effects", "Interaction (SAT x Public)", "Interaction (SAT x Public$^2$)")) %>%
  cbind(rbind(glance(lm.3), glance(lm.4), glance(lm.5))) %>%
  select(Model, R2 = r.squared, df1 = df, df2 = df.residual, F = statistic, p = p.value) %>%
...
$$
\begin{split}
\mathbf{Model~A:~} Y_i&=\beta_0 + \beta_1(X_{1i}) + \beta_2(X_{2i}) + \epsilon_i\\
\mathbf{Model~B:~} Y_i&=\beta_0 + \beta_1(X_{1i}) + \beta_2(X_{2i}) + \beta_3(X_{1i})(X_{2i}) + \epsilon_i
\end{split}
$$

Model A is said to be *nested in* Model B as its terms are a subset of the terms in Model B. In general a model is nested in another model if setting a subset of the coefficients of the more complex model to zero results in the simpler model. In this example, Setting $\beta_3$ in Model B (the more complex model) to 0 results in Model A (the simpler model).
...
Here Model 3 is nested within Model 4, which is, in turn, nested in Model 5. Because of this we can use an *F*-test to compare the change in $R^2$ between the different models. Because we are evaluating the *change in* $R^2$, sometimes this test is referred to as the *Delta F-test*, or symbolically, the $\Delta F$-test. 
