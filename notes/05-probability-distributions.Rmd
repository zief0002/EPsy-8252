...
To illustrate how probability distributions are used in practice, we will will use the *riverside.csv* (see the [data codebook](#riverside) here) and fit a regression model that uses education level and seniority to predict variation in employee income.

```{r message=FALSE, warning=FALSE}
# Read in data
city = read_csv(file = "~/Documents/github/epsy-8252/data/riverside.csv")
head(city)
...
We can also compute the model-level $F$-statistic using the partititioning of variation from the ANOVA table.
