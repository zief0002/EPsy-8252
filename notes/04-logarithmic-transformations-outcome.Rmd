...
```{r eval=FALSE, fig.width=12, fig.height=4, out.width='6in', fig.cap="Distributions of the Outcome and Predictor"}
...
```{r echo=FALSE, fig.width=12, fig.height=4, out.width='6in', fig.cap="Distributions of the Outcome and Predictor"}
# Marginal distribution of budget (outcome)
p1 = ggplot(data = movies, aes(x = budget)) +
  stat_density() +
  theme_bw() +
...
*Note.* Density plot of the distribution of movie budgets (left) and running time (center). The scatterplot (right) shows the relationship between running time and budget. The loess smoother  (blue, dashed line) is also displayed on the scatterplot. All movies that have a budget over 400 million dollars or a running time of at least three hours are also identified.
...
```{r fig.cap="Residual Plots from Model 1", fig.width=8, fig.height=4, out.width='6in'}
...
```{r fig.cap='Scatterplot between Age and Log-Transformed Budget', out.width='4in'}
# Scatterplot
...
*Note.* Movies with extreme running times or budgets are identified. The loess smoother (blue, dashed line) is also displayed.
...
```{r fig.cap="Residual Plots for Model 2", fig.width=8, fig.height=4, out.width='6in'}
# Density plot of the residuals
...
```{r out.width='3in', fig.cap='Plot of Predicted Budget as a Function of Running Time'}
# Plot
...
```{r fig.width=8, fig.height=4, out.width="6in", fig.cap="Standardized Residuals versus the Fitted Values for Two Competing Nonlinear Models"}
# Log-log residuals
...
```{r fig.width=8, fig.height=4, fig.cap='Scatterplot between Age and Log-Transformed budget and Predicted Curve for Two Competing Nonlinear Models', out.width='6in'}
# Log-log model
...
```{r out.width='3in', fig.cap='Plot of Predicted Budget as a Function of Running Time for the Polynomial Model'}
# Plot
ggplot(data = movies, aes(x = length, y = budget)) +
  geom_point(alpha = 0) +
...
```{r fig.cap='Jittered Scatterplot of Log-Transformed Budget versus Action Movie', out.width='3in'}
...
*Note.* The fitted curves for both action movies (red, solid line) and non-action movies (black, dashed line) are displayed.
