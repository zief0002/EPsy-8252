...
The fitted equation expresses graduation rates as a function of base-2 log-transformed median SAT scores. Alternatively, we can re-write this equation to express graduation rates as a function of raw median SAT scores. To understand how to do this, consider the general mathematical expression of the fitted equation:
...
We can also use this alternative expression in the `stat_function()` layer of our `ggplot()` syntax. 
...
As with any base, using base-$e$ results in the same model-level evidence ($R^2=.811$, $F(1,~31)=133.3$, $p<.001$).
